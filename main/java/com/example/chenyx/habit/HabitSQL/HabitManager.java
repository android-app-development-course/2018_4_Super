...
public class HabitManager   //日期数据库操作类
{
    ...
    public Boolean find(int year,int month,int day) //在数据中查找某天
    {
        ...
        Cursor  cursor=db.query(mysigndatahelper.table_name,null,"year=? and month=? and day=?",new String[]{""+year,""+month,""+day},null,null,null);
    ...
    public void update(int year,int month,int day) //将某天最长连签时间+1
    {
        ...
        db.update(mysigndatahelper.table_name,values,"year=? and month=? and day=?",new String[]{""+year,""+month,""+day});
        db.close();
    }
    ...
    public long insert() //自动插入当前日期
    {
        ...
        values.put("day",day);
        values.put("max_sign_day",max_sign_day);
        long id=db.insert(mysigndatahelper.table_name,null,values);
        db.close();
    ...
    {   //返回光标
        SQLiteDatabase db=mysigndatahelper.getReadableDatabase();
        Cursor cursor=db.query(mysigndatahelper.table_name,null,null,null,null,null,null);
        //db.close();
    ...
    {   //按data_id删除某天
        SQLiteDatabase db=mysigndatahelper.getWritableDatabase();
        db.delete(mysigndatahelper.table_name,"data_id=?",new String[]{""+id});
        db.close();
    }
    ...
    {   //按日期删除某天
        SQLiteDatabase db=mysigndatahelper.getWritableDatabase();
        db.delete(mysigndatahelper.table_name,"year=? and month=? and day=?",new String[]{""+year,""+month,""+day});
        db.close();
    }
